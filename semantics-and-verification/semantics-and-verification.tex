...
We now consider a simple imperative language: IMP. Variables are in a fixed set: $X\in\mathbb{V}$. Constants are defined as a set $\mathbb{I} := \mathbb{B}\cup\Z$, containing booleans $\mathbb{B}:=\{\true, \false\}$ and integers $\Z$. Furthermore, we include unary and binary operators, represented by $\lozenge$. Such operators can be integer operations $+, -, \times, /, <, \leq$, boolean operations $\lnot, \land, \lor$, or polymorphic operations $=, \neq$.
\begin{figure}[H]
    \begin{minipage}{0.49\textwidth}
        \centering
        \begin{mgrammar}
