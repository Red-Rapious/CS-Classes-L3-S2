...
\begin{definition}[Traces]
    A \emph{finite trace} is a finite sequence of states $s_0, \dots, s_n$ noted $\langle s_0, \dots, s_n\rangle$. An infinite trace is an infinite sequence of states $\langle s_0, \dots\rangle$. Besides, we write $\S^\star$ for the set of finite traces, $\S^\omega$ for the set of infinite traces, and $S^\alpha=\S^\star\cup\S^\omega$.
\end{definition}
...
\begin{definition}[Prefix closure]
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            X &\mapsto \{\sigma_{\lceil i} \: | \: \sigma\in X, i\in \N\}
        \end{aligned}
    \end{equation*}
\end{definition}
...
\begin{definition}[The $\Lim$ operator]
    ...
    \begin{aligned}
        ...
        X &\mapsto X\cup \{\sigma\in\S^\alpha \: | \: \forall i\in\N, \sigma_{\lceil i}\in X\}
    \end{aligned}
    \end{equation*}
...
\begin{definition}{The $\safe$ operator}
    Operator $\safe$ is defined by $\safe = \Lim\circ\pcl$.\\
    Operator $\safe$ is an upper closure operator over $\mathcal{P}(\S^\alpha)$.
\end{definition}

\begin{definition}[Safety property]
    A trace property $\mathcal{T}$ is a safety property if and only if $\safe(\mathcal{T}) = \mathcal{T}$. If $\mathcal{T}$ is a trace property, then $\safe(\mathcal{T})$ is a safety property.
\end{definition}
