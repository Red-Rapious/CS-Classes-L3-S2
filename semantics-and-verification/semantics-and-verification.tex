...
\begin{definition}[Operator $\live$]
    ...
    \begin{equation*}
        \live(\T) = \T \cup (\S^\alpha \setminus \safe(\T))
    \end{equation*}
\end{definition}
...
\begin{definition}[Liveness property]
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item $\Lim \circ \pcl(\T) = \S^\alpha$
    \end{enumerate}
\end{definition}
\begin{proof}[Proof: $(i) \implies (ii)$]
    We assume that $\live(\T) = \T$, i.e. that $\T\cup(\S^\alpha\setminus\safe(\T)) = \T$. Therefore, $\S^\alpha\setminus\safe(\T) \subseteq \T$.

    Let $\sigma\in\S^\star$ and let us show that $\sigma\in\pcl(\T)$. Since we must have $\sigma\in\S^\alpha$:
    \begin{itemize}
...
\begin{proof}[Proof: $(ii) \implies (iii)$]
    If $\pcl{\T}=\S^\star$, then $\Lim\circ\pcl(\T) = \S^\alpha$.
\end{proof}
\begin{proof}[Proof: $(iii) \implies (i)$]
    If $\Lim\circ\pcl(\T) = \S^\alpha$, then
    \begin{equation*}
        \live(\T) = \T\cup(\S^\alpha\setminus(\Lim\circ\pcl(\T))) = \T \cup (\S^\alpha\setminus\S^\alpha) = \T
    \end{equation*}
\end{proof}
...
\begin{proof}
    ...
    \begin{equation*}
        \begin{aligned}
            \pcl\circ\live(\T) &= \pcl(\T)\cup\pcl(\S^\alpha\setminus\safe(\T)) \\
            &= \pcl(\T)\cup\pcl(\S^\alpha\setminus\Lim\circ\pcl(\T))
        \end{aligned}
    \end{equation*}
    If $\sigma\in\pcl(\T)$, the result is obvious. If $\sigma\notin\pcl(\T)$, then $\sigma\notin\Lim\circ\pcl(\T)$ by definition of the limit, thus $\sigma\in\S^\alpha\setminus\Lim\circ\pcl(\T)$. Therefore, $\sigma\in\pcl(\S^\alpha\setminus\Lim\circ\pcl(\T))$ as $\pcl$ is extensive when applied to sets of finite traces, which proves the above result.
\end{proof}

\subsubsection{Termination proof with ranking function}
In this section, we will only consider termination. Given a transition system $\Sc=(\S, \to, \S_{\I})$, and a liveness property $\T$, we look for a way of verifying that $\Sc$ satisfies termination, i.e. that $\sem{\Sc}^\alpha\subseteq\S^\star$. To do so, we introduce the notion of ranking function.
