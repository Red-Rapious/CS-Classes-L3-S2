...
\begin{figure}[H]
    \begin{minipage}{0.49\textwidth}
        ...
        \begin{mgrammar}
            ...
            \grule{c}{constant}
            \grule{\diamond\,\expr}{unary op.}
            \grule{\expr \,\diamond\, \expr}{binary op.}
        \end{mgrammar}
    \end{minipage}
    \begin{minipage}{0.49\textwidth}
        \centering
...
\begin{definition}[Flat ordering]
...
\begin{definition}[Poset of continuous partial functions]
...
Finally, we can introduce the \emph{assignment axiom}: for $P$ over $X$ to be true after $X \leftarrow e$, $P$ must be true over $e$ before the assignment:
\begin{figure}[H]
    \centering
    \begin{prooftree}
        \hypo{}
...
\begin{example}[Consequence rule]
    The axiom for \textnormal{\textbf{fail}} can be replaced with:
    \begin{figure}[H]
        \centering
        \begin{prooftree}
            \hypo{}
            \infer1[Ax.]{\hoare{\true}{\textnormal{\bf fail}}{\false}}
        \end{prooftree}
    \end{figure}
...
\begin{example}[Test rule]
    \leavevmode
    \begin{figure}[H]
        \centering
        \begin{prooftree}
            \hypo{}
            \infer1[Ax.]{\hoare{X<0}{X\leftarrow -X}{X>0}}
            \infer1[Weak.]{\hoare{(X\neq 0)}{X\leftarrow -X}{X>0}}
            \hypo{}
            \infer1[Ax.]{\hoare{X>0}{\textnormal{\bf skip}}{X>0}}
            \infer1[Weak.]{\hoare{(X\neq 0) \land (X\geq0)}{\textnormal{\bf skip}}{X>0}}
            \infer2[Test]{\hoare{X\neq 0}{\textnormal{\textbf{if} $X<0$ \textbf{then} $X\leftarrow -X$ \textbf{else skip}}}{X>0}}
...
In practice, we would rather prove the triple $\hoare{P}{\textnormal{\textbf{while} e \textbf{do} s}}{Q}$. To do so, it is sufficient to invent an assertion $I$ that:
\begin{itemize}
...
\begin{figure}[H]
    ...
    \begin{prooftree}
        \hypo{P\implies I}
        \hypo{I\land\lnot e\implies Q}
        \hypo{\hoare{I\land e}{s}{I}}
        \infer1{\hoare{I}{\textnormal{\textbf{while} e \textbf{do} s}}{I\land\lnot e}}
        \infer3[Inv.]{\hoare{P}{\textnormal{\textbf{while} e \textbf{do} s}}{Q}}
    \end{prooftree}
\end{figure}
...
\begin{definition}[Weakest liberal preconditions]
    ...
    \end{equation*}
    \say{Liberal} means that we do not care about termination and errors.
\end{definition}
...
\begin{definition}[Calculus definition for \emph{wlp}]
    \begin{equation*}
        \begin{aligned}
...
\begin{property}[Excluded miracle]
    \begin{equation*}
        \wlp(c, \false) \equiv \false
    \end{equation*}
\end{property}

\begin{property}[$\land$-distributivity]
    \begin{equation*}
        \wlp(c, P) \land \wlp(d, Q) \equiv \wlp(c, P\land Q)
    \end{equation*}
\end{property}

\begin{property}[$\lor$-distributivity]
    \begin{equation*}
        \wlp(c, P) \lor \wlp(d, Q) \equiv \wlp(c, P\lor Q)
    \end{equation*}

    Note that $\implies$ is always true, while $\impliedby$ is only true for deterministic, error-free programs.
\end{property}
...
\begin{definition}[Calculus definition for \emph{slp}]
    \begin{equation*}
        \begin{aligned}
...
\begin{figure}[H]
    \centering
    \begin{grammar}
        \firstrule{stat}{X $\leftarrow$ expr}{}
        \grule{\bf skip}{}
        \grule{\bf fail}{}
        \grule{stat; stat}{}
        \grule{\textbf{if} expr \textbf{then} stat \textbf{else} stat}{}
        \grule{\textbf{while} expr \textbf{do} stat}{}
        \grule{\textbf{assert} expr}{assertion}
        \gskip{}
        \firstrule{prog}{$\hoare{\textnormal{Prop}}{\textnormal{stat}}{\textnormal{Prop}}$}{}
    \end{grammar}
...
\begin{figure}[H]
    \centering
    \begin{prooftree}
        \hypo{\forall t\in W, \; \sqhoare{P\land e\land u=t}{s}{P\land u \prec t}}
        \infer1[Loop]{\sqhoare{P}{\textnormal{\textbf{while} e \textbf{do} s}}{P\land\lnot e}}
    \end{prooftree}    
\end{figure}
in which we require $(W, \prec)$ to be a well-founded ordering. This ensures that we cannot decrease infinitely by $\prec$ in $W$. In general, we simply use $(\N, <)$, even though lexicographic orders and ordinals can also come handy. Intuitively, in addition to the loop invariant $P$, we invent an expression $u$ that strictly decreases by $s$. Such a $u$ is called a \emph{ranking function}; often, $\lnot e\implies u=0$: $u$ counts the number of steps until termination.
...
\begin{figure}[H]
    ...
    \begin{prooftree}
        \hypo{\hoare{P}{c}{Q}}
        \hypo{\sqhoare{P}{c}{\true}}
        \infer2{\sqhoare{P}{c}{Q}}
    \end{prooftree}
\end{figure}
...
\begin{example}
    ...
    \begin{figure}[H]
        \centering
        \begin{prooftree}
            \hypo{\forall n\in \N, \; \sqhoare{P\land e\land r=n}{s}{P\land r<n}}
            \hypo{(P\land e)\implies(r\geq0)}
            \infer2[Loop]{\sqhoare{P}{\textnormal{\textbf{while} e \textbf{do} s}}{P\land\lnot e}}
        \end{prooftree}    
    \end{figure}
    ...
    \begin{figure}[H]
        \centering
        \begin{prooftree}
            \hypo{\forall n\in \N, \; \sqhoare{I<N\land N-I=n}{s}{N-I=n-1}}
            \hypo{(I<N)\implies(N-I\geq0)}
            \infer2[Loop]{\sqhoare{\true}{p}{I\geq N}}
        \end{prooftree}
    \end{figure}
\end{example}
...
\begin{figure}[H]
    ...
    \begin{prooftree}
        \hypo{}
        \infer1[Ax.]{\hoare{\forall X, P}{X\leftarrow\;?}{P}}
    \end{prooftree}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{prooftree}
        \hypo{}
        \infer1[Ax.]{\hoare{P}{X\leftarrow\;?}{P}}
    \end{prooftree}
\end{figure}
...
\begin{example}
    ...
    \begin{figure}[H]
        ...
        \begin{prooftree}
            \hypo{}
            \infer1{\hoare{X=x}{Y\leftarrow X}{Y=x}}
            \hypo{}
            \infer1{\hoare{Y=x}{X\leftarrow\;?}{Y=x}}
            \hypo{}
            \infer1{\hoare{Y=x}{X\leftarrow Y}{X=x}}
            \infer3[Seq.]{\hoare{X=x}{X\leftarrow X; X\leftarrow\;?; X\leftarrow Y}{X=x}}
        \end{prooftree}
    \end{figure}
\end{example}
...
\begin{figure}[H]
    ...
    \begin{prooftree}
        \hypo{}
        \infer1[Ax.]{\hoare{P[f/A(e)]}{A(e)\leftarrow f}{P}}
    \end{prooftree}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{prooftree}
        \hypo{}
        \infer1[Ax.]{\hoare{P[A\{e\mapsto f\}/A]}{A(e)\leftarrow f}{P}}
    \end{prooftree}
\end{figure}
...
\begin{figure}[H]
    \centering
    \begin{prooftree}
        \hypo{\hoare{P_1}{s_1}{Q_1}}
        \hypo{\hoare{P_2}{s_2}{Q_2}}
        \infer2[Conc.]{\hoare{P_1\land P_2}{s_1 || s_2}{Q_1\land Q_2}}
    \end{prooftree}
\end{figure}
...
\begin{example}
    ...
    \begin{figure}[H]
        ...
        \begin{prooftree}
            \hypo{}
            \infer1{\hoare{X=Y=0}{s_1}{X=1\land Y=0}}
            \hypo{}
            \infer1{\hoare{X=Y=0}{s_2}{X=0\land Y=1}}
            \infer2[Conc.]{\hoare{X=Y=0}{s_1 || s_2}{\false}}
        \end{prooftree}
    \end{figure}
\end{example}
...
\begin{definition}[Interference freedom]
...
\begin{example}
    ...
    \begin{figure}[H]
        ...
        \begin{prooftree}
            \hypo{}
            \infer1{\hoare{X=0\land Y\in[0, 1]}{s_1}{X=1\land Y\in[0, 1]}}
            \hypo{}
            \infer1{\hoare{X\in[0, 1]\land Y=0}{s_2}{X\in[0, 1]\land Y\in[0, 1]}}
            \infer2{\hoare{X=Y=0}{s_1 ||\;s_2}{X=1\land Y\in[0, 1]}}
        \end{prooftree}
    \end{figure}
\end{example}
...
\begin{theorem}[Static strong typing]
...
\begin{example}[Javascript, a dynamically typed safe language]
...
\begin{figure}[H]
    ...
    \begin{grammar}
        \firstrule{expr}{X}{variable}
        \grule{c}{constant}
        \grule{$\diamond$ expr}{unary operation}
        \grule{expr $\diamond$ expr}{binary operation}
        ...
        \grule{\textbf{if} expr \textbf{then} stat \textbf{else} stat}{conditional}
        \grule{\textbf{while} expr \textbf{do} stat}{loop}
        \grule{\textbf{local} X \textbf{in} stat}{loop}
    \end{grammar}
\end{figure}
...
A deductive system is a set of axioms and logical rules to derive theorems, which define what is provable in a formal way. We introduce  \emph{judgments} $\Gamma\vdash \textnormal{Prop}$, a fact meaning \say{under hypotheses $\Gamma$, we can prove Prop}. Similarly to Hoare, logic, we structure rules (hoptheses-conclusion rules, and axioms) into proof trees, which complete application of rules from axioms to conclusion.
...
\begin{figure}[H]
    \centering
    \begin{grammar}
        \firstrule{type}{\textbf{int}}{integers}
        \grule{\textbf{bool}}{booleans}
    \end{grammar}
\end{figure}
...
\begin{definition}[Well-typed statement]
...
\begin{theorem}
    Well-typed program \say{cannot go wrong}.
\end{theorem}
...
An approach is bottom-up reasoning: we construct a proof tree ending in $\emptyset\vdash s$ by applying rules \say{in reverse}. Given a conclusion, there is generally only one rule to apply. The only rule that requires imagination is
\begin{figure}[H]
    \centering
    \begin{prooftree}
        \hypo{\Gamma\cup\{(X:t)\}\vdash s}
...
\begin{figure}[H]
    \centering
    \begin{grammar}
        \firstrule{stat}{\textbf{local} $X:\textnormal{type}$ \textbf{in} stat}{}
        \grule{\dots}{}
    \end{grammar}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{prooftree}
        \hypo{\Gamma\cup\{(X:t)\}\vdash s}
        \infer1{\Gamma\vdash\textbf{local } X:t \textbf{ in } s}
    \end{prooftree}
\end{figure}
...
\begin{figure}[H]
    \centering
    \begin{grammar}
        \firstrule{type constr}{type expr = type expr}{type equality}
        \gskip
        \firstrule{type expr}{\bf int}{integers}
        \grule{\bf bool}{booleans}
        \grule{$t_X$}{type variable for $X\in\V$}
    \end{grammar}
\end{figure}
